...
from yt_dlp_mod.constants import (
    videoExtensions,
    audioExtensions,
    videoQualities,
    audioQualities,
...
from yt_dlp_mod.exceptions import (
    UserInputError,
    FileSizeOutOfRange,
...
from yt_dlp_mod.utils import sanitize_filename
...
class YoutubeDLMod(YoutubeDL):
    ...
    def __enter__(self) -> "YoutubeDLMod":
    ...
    def get_format_quality(
        ...
        if info_format.format_note:
            # Check if it's already a standard quality
            if info_format.format_note in mediaQualities:
                return info_format.format_note
            
            # Try to extract it from notes like "1080p, AI-upscaled" or "720p60"
            match = re.search(r"(\d{3,4}p(\d{2})?)", info_format.format_note)
            if match:
                quality = match.group(1)
                if quality in mediaQualities:
                    return quality
    ...
    def process_extracted_info(
        ...
        for format in extracted_info.formats:
            # Normalize format_note even if it already exists
            normalized_quality = self.get_format_quality(format)
            if normalized_quality:
                format.format_note = normalized_quality
            ...
            if filter_best_protocol:
                if format.protocol == target_format_protocol:
                    ...
                    if format.format_note: # If we have a note (standardized)
                        # https
    ...
    def extract_info_and_form_model(
        ...
        if extracted_info.get("_type") == "playlist" and extracted_info.get("entries"):
            # For convenience in download-video/audio, if search result, take first entry
...
class Downloader(PostDownload):
    ...
    def ydl_run(
        ...
        # Robustly extract filepath
        filepath = None
        if processed_info_dict.get("requested_downloads"):
            filepath = processed_info_dict["requested_downloads"][0].get("filepath")
        if not filepath:
            filepath = processed_info_dict.get("_filename")
            
        processed_info_dict["filepath"] = filepath
